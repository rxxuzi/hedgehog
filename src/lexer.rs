        ...
        if first == '0' {
            match self.peek() {
                Some('x') | Some('X') => {
                    ...
                    let value = i64::from_str_radix(&num_str, 16).map_err(|_| LexerError {
                        message: "Invalid hex number".to_string(),
                        span: self.current_span(),
                    })?;
                ...
                Some('o') | Some('O') => {
                    ...
                    let value = i64::from_str_radix(&num_str, 8).map_err(|_| LexerError {
                        message: "Invalid octal number".to_string(),
                        span: self.current_span(),
                    })?;
                    return Ok(Token::Int(value));
                ...
                Some('b') | Some('B') => {
                    ...
                    let value = i64::from_str_radix(&num_str, 2).map_err(|_| LexerError {
                        message: "Invalid binary number".to_string(),
                        span: self.current_span(),
                    })?;
                    return Ok(Token::Int(value));
...
mod tests {
    ...
    fn test_bind() {
        assert_eq!(
            tokenize("^= x 42"),
            vec![Token::Bind, Token::Ident("x".to_string()), Token::Int(42),]
        );
    }
    ...
    fn test_output() {
        assert_eq!(
            tokenize("~> \"hello\""),
            vec![Token::Out, Token::String("hello".to_string()),]
        );
    }
    ...
    fn test_output_raw() {
        assert_eq!(
            tokenize("~| \"test\""),
            vec![Token::OutRaw, Token::String("test".to_string()),]
        );
    }
    ...
    fn test_output_err() {
        assert_eq!(
            tokenize("~! \"error\""),
            vec![Token::OutErr, Token::String("error".to_string()),]
        );
    }
    ...
    fn test_cond() {
        assert_eq!(
            tokenize("?: true 1 0"),
            vec![Token::Cond, Token::True, Token::Int(1), Token::Int(0),]
        );
    }
    ...
    fn test_loop() {
        assert_eq!(
            tokenize("?@ true ~> 1"),
            vec![Token::Loop, Token::True, Token::Out, Token::Int(1),]
        );
    }
    ...
    fn test_range() {
        assert_eq!(
            tokenize("%.. 1 10"),
            vec![Token::Range, Token::Int(1), Token::Int(10),]
        );
    }
    ...
    fn test_arithmetic() {
        assert_eq!(
            tokenize(".+ 1 2"),
            vec![Token::Add, Token::Int(1), Token::Int(2)]
        );
        assert_eq!(
            tokenize(".- 5 3"),
            vec![Token::Sub, Token::Int(5), Token::Int(3)]
        );
        assert_eq!(
            tokenize(".* 4 5"),
            vec![Token::Mul, Token::Int(4), Token::Int(5)]
        );
        assert_eq!(
            tokenize("./ 10 2"),
            vec![Token::Div, Token::Int(10), Token::Int(2)]
        );
        assert_eq!(
            tokenize(".% 10 3"),
            vec![Token::Mod, Token::Int(10), Token::Int(3)]
        );
        assert_eq!(
            tokenize(".^ 2 8"),
            vec![Token::Pow, Token::Int(2), Token::Int(8)]
        );
    }
    ...
    fn test_comparison() {
        assert_eq!(
            tokenize(".= 1 1"),
            vec![Token::Eq, Token::Int(1), Token::Int(1)]
        );
        assert_eq!(
            tokenize(".~ 1 2"),
            vec![Token::Neq, Token::Int(1), Token::Int(2)]
        );
        assert_eq!(
            tokenize(".< 1 2"),
            vec![Token::Lt, Token::Int(1), Token::Int(2)]
        );
        assert_eq!(
            tokenize(".> 2 1"),
            vec![Token::Gt, Token::Int(2), Token::Int(1)]
        );
        assert_eq!(
            tokenize(".<= 1 1"),
            vec![Token::Lte, Token::Int(1), Token::Int(1)]
        );
        assert_eq!(
            tokenize(".>= 2 1"),
            vec![Token::Gte, Token::Int(2), Token::Int(1)]
        );
    }
    ...
    fn test_logical() {
        assert_eq!(
            tokenize(".& true false"),
            vec![Token::And, Token::True, Token::False]
        );
        assert_eq!(
            tokenize(".| true false"),
            vec![Token::Or, Token::True, Token::False]
        );
    ...
    fn test_raw_string() {
        assert_eq!(
            tokenize("'hello'"),
            vec![Token::RawString("hello".to_string()),]
        );
    }
    ...
    fn test_command_string() {
        assert_eq!(
            tokenize("`echo hello`"),
            vec![Token::CommandString("echo hello".to_string()),]
        );
    }
    ...
    fn test_identifier_with_bang() {
        assert_eq!(
            tokenize("unwrap!"),
            vec![Token::Ident("unwrap!".to_string())]
        );
    }
    ...
    fn test_block_comment() {
        assert_eq!(tokenize("#- block comment -#42"), vec![Token::Int(42)]);
    }
    ...
    fn test_lib_import() {
        assert_eq!(
            tokenize("/+ std"),
            vec![Token::LibImport, Token::Ident("std".to_string()),]
        );
    }
    ...
    fn test_file_read() {
        assert_eq!(
            tokenize("<+ \"file.txt\""),
            vec![Token::FileRead, Token::String("file.txt".to_string()),]
        );
    }
    ...
    fn test_binary_read() {
        assert_eq!(
            tokenize("<* \"data.bin\""),
            vec![Token::BinaryRead, Token::String("data.bin".to_string()),]
        );
    }
    ...
    fn test_struct_def() {
        assert_eq!(
            tokenize("^- Point"),
            vec![Token::StructDef, Token::Ident("Point".to_string()),]
        );
    }
    ...
    fn test_global_ref() {
        assert_eq!(
            tokenize("$>config"),
            vec![Token::GlobalRef, Token::Ident("config".to_string()),]
        );
    }
    ...
    fn test_parent_ref() {
        assert_eq!(
            tokenize("$<x"),
            vec![Token::ParentRef, Token::Ident("x".to_string()),]
        );
    }
    ...
    fn test_interp_escape() {
        assert_eq!(
            tokenize("\"\\$100\""),
            vec![Token::String("$100".to_string())]
        );
    }
    ...
    fn test_let_in() {
        assert_eq!(
            tokenize(";= x 10"),
            vec![Token::LetIn, Token::Ident("x".to_string()), Token::Int(10),]
        );
    }
    ...
    fn test_drop() {
        assert_eq!(
            tokenize(";; x"),
            vec![Token::Drop, Token::Ident("x".to_string()),]
        );
    }
    ...
    fn test_cmd_check() {
        assert_eq!(
            tokenize("!? \"git\""),
            vec![Token::CmdCheck, Token::String("git".to_string()),]
        );
    }

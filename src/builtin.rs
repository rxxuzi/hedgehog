    ...
    match name {
        ...
        "head" => {
            ...
            match &args[0] {
                Value::List(l) => l
                    .first()
                    .cloned()
                    .ok_or_else(|| EvalError::new("head on empty list", line, col)),
                Value::String(s) => s
                    .chars()
                    .next()
        ...
        "last" => {
            ...
            match &args[0] {
                Value::List(l) => l
                    .last()
                    .cloned()
                    .ok_or_else(|| EvalError::new("last on empty list", line, col)),
                Value::String(s) => s
                    .chars()
                    .last()
                    .map(|c| Value::String(c.to_string()))
        ...
        "range" => {
            ...
            match (&args[0], &args[1], &args[2]) {
                ...
                _ => Err(EvalError::new(
                    "range requires 3 integers: start, end, step",
                    line,
                    col,
                )),
            }
        ...
        "concat" => {
            ...
            match (&args[0], &args[1]) {
                ...
                _ => Err(EvalError::new(
                    "concat requires two lists or strings",
                    line,
                    col,
                )),
            }
        ...
        "contains" => {
            ...
            match (&args[0], &args[1]) {
                ...
                (Value::List(l), val) => Ok(Value::Bool(l.contains(val))),
                _ => Err(EvalError::new(
                    "contains requires string/string or list/value",
                    line,
                    col,
                )),
            }
        ...
        "float" => {
            ...
            match &args[0] {
                ...
                Value::String(s) => s.parse::<f64>().map(Value::Float).map_err(|_| {
                    EvalError::new(format!("Cannot parse '{}' as float", s), line, col)
                }),
        ...
        "unwrap" => {
            ...
            match &args[0] {
                ...
                _ => Err(EvalError::new(
                    "unwrap requires Option or Result",
                    line,
                    col,
                )),
            }
        ...
        "unwrap-or" => {
            ...
            match &args[0] {
                ...
                _ => Err(EvalError::new(
                    "unwrap-or requires Option or Result",
                    line,
                    col,
                )),
            }
...
mod tests {
    ...
    fn test_int_from_bool() {
        ...
        assert_eq!(
            call("int", vec![Value::Bool(false)]).unwrap(),
            Value::Int(0)
        );
    }
    ...
    fn test_str() {
        assert_eq!(
            call("str", vec![Value::Int(42)]).unwrap(),
            Value::String("42".to_string())
        );
        assert_eq!(
            call("str", vec![Value::Bool(true)]).unwrap(),
    ...
    fn test_bool() {
        assert_eq!(
            call("bool", vec![Value::Int(0)]).unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            call("bool", vec![Value::Int(1)]).unwrap(),
            Value::Bool(true)
        );
        assert_eq!(
            call("bool", vec![Value::String("".to_string())]).unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            call("bool", vec![Value::String("x".to_string())]).unwrap(),
            Value::Bool(true)
        );
    ...
    fn test_some_check() {
        assert_eq!(
            call("some?", vec![Value::Some(Box::new(Value::Int(1)))]).unwrap(),
            Value::Bool(true)
        );
        assert_eq!(
            call("some?", vec![Value::None]).unwrap(),
            Value::Bool(false)
        );
    }
    ...
    fn test_none_check() {
        ...
        assert_eq!(
            call("none?", vec![Value::Some(Box::new(Value::Int(1)))]).unwrap(),
            Value::Bool(false)
        );
    }
    ...
    fn test_ok_check() {
        assert_eq!(
            call("ok?", vec![Value::Ok(Box::new(Value::Int(1)))]).unwrap(),
            Value::Bool(true)
        );
        assert_eq!(
            call("ok?", vec![Value::Err(Box::new(Value::Int(1)))]).unwrap(),
            Value::Bool(false)
        );
    }
    ...
    fn test_err_check() {
        assert_eq!(
            call("err?", vec![Value::Err(Box::new(Value::Int(1)))]).unwrap(),
            Value::Bool(true)
        );
        assert_eq!(
            call("err?", vec![Value::Ok(Box::new(Value::Int(1)))]).unwrap(),
            Value::Bool(false)
        );
    }
    ...
    fn test_unwrap_or_some() {
        let result = call(
            "unwrap-or",
            vec![Value::Some(Box::new(Value::Int(42))), Value::Int(0)],
        );
        assert_eq!(result.unwrap(), Value::Int(42));
    }
